...
directory = '/data1/apacheRoot/html/shine/ReportArchieve/Daily_Report_Digital_Mailer' + '/' + str(Year) + '/' + str(Month) + '/' 
...
def mail_file(file):
    mailing_list = ['venu@digitalmailers.in','sunilk@digitalmailers.in']
...
def getDataFromMongo():
    Output_File = directory + "Daily_Report_Digital_Mailer_" + str(file_date) + ".csv" 
    ofile = open(Output_File ,'wb')
    writer = csv.writer(ofile)
    writer.writerow(['User_Id','Vendor_Id','Experience'])
    Required_Data = collection.find({"evi": {"$in": [8805,8806,8807]},'red':{'$gte':day1,'$lt':day2},'rm':0,'mo':0},{"_id": 1,"evi": 1,"ex": 1})
