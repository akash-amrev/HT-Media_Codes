 ...
 */
public class NPhase1_v02 extends Configured implements Tool {
	...
	public static class Reduce extends MapReduceBase implements
		...
		public static float distance(float[] p1, float[] p2, float skillCutoff) {
			...
			for (int i = 0; i < p1.length; i++) {
				if (p1[i] > skillCutoff){				// Distance Measure only if similarity is above skillCutoff
		...
		public void reduce(IntWritable key, Iterator<NPhase1Value> values,
			...
			while (values.hasNext()) {
				// Spilling of Records to local disk. Each Reducer will dump its record
			...
			while (flag) {
				...
				while (true) {
					...
					for (int i = 0; i < number; i++) {
						...
						simval = distance(x_skill, y_skill, skillCutoff);
	...
	public int run(String[] args) throws Exception {
		...
		JobConf conf = new JobConf(getConf(), NPhase1_v02.class);
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new NPhase1_v02(), args);
		System.exit(res);
	}
} // NPhase1