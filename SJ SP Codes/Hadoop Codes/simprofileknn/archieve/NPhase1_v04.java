 ...
 * 
 * This Version - Added skill skillCutoff and weightedCutoff parameter to restrict the computation to limited skills
 * Few Performance Enhancements to release memory.
 * Changed the skill distance measure (skill similarity without method call so that no skill array copy needed)
 * Adding Jacard similarity for skills to existing cosine similarity
 */
 ...
 */
public class NPhase1_v04 extends Configured implements Tool {
			...
			while (flag) {
				...
				while (true) {
					...
					for (int i = 0; i < number; i++) {
						...
						simvalCosine = (float) Math.pow(simvalCosine, 4);
						...
						simval = ((0.7f * simvalCosine) + (0.3f * simvalJacard));
	...
	static int printUsage() {
		System.out
				.println("NPhase1 [-m <maps>] [-r <reduces>] [-p <numberOfPartitions>] "
						+ "[-d <dimension>] [-k <knn>] [-t <weightedCutoff>] [-c <skillCutoff>] "
						+ "[-b <blockSize(#records) for R>] <input (R)> <input (S)> <output> <weights>");
		System.out
	...
	public int run(String[] args) throws Exception {
		...
		JobConf conf = new JobConf(getConf(), NPhase1_v04.class);
	...
	public static void main(String[] args) throws Exception {
		int res = ToolRunner.run(new Configuration(), new NPhase1_v04(), args);
		System.exit(res);
	}
} // NPhase1