...
directory = '/data1/apacheRoot/html/shine/ReportArchieve/Daily_Report_Lucini' + '/' + str(Year) + '/' + str(Month) + '/' 
...
def mail_file(file):
    mailing_list = ['jitendra.singh@lucinilucini.com']
...
def getDataFromMongo():
    Output_File = directory + "Daily_Report_Lucini_" + str(file_date) + ".csv" 
    ofile = open(Output_File ,'wb')
    writer = csv.writer(ofile)
    writer.writerow(['User_Id','Vendor_Id','Experience','Email_Verified'])
    Required_Data = collection.find({"evi": {"$in": [8705, 8706]},'red':{'$gte':day1,'$lt':day2},'rm':0,'mo':0},{"_id": 1,"evi": 1,"ex": 1,"ev":1})
