...
directory = '/data1/apacheRoot/html/shine/ReportArchieve/Daily_Report_Lucini_OTP' + '/' + str(Year) + '/' + str(Month) + '/' 
...
def mail_file(file):
    ...
    Content = 'PFA the Daily Report For Lucini Vendor(OTP) \n\n Regards \n Akash'
    attachment = [file]
    sendMailUtil.send_mail(mailing_list ,cc_list, bcc_list, Subject ,Content,attachment ,0)

def getDataFromMongo():
    Output_File = directory + "Daily_Report_Lucini_OTP_" + str(file_date) + ".csv" 
    ofile = open(Output_File ,'wb')
    writer = csv.writer(ofile)
    writer.writerow(['User_Id','Vendor_Id','Experience'])
    Required_Data = collection.find({"evi": {"$in": [90011]},'red':{'$gte':day1,'$lt':day2},'rm':0,'mo':0},{"_id": 1,"evi": 1,"ex": 1})
